Shader "Hidden/Custom/LightPost"
{
  Properties
  {
  }
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat0_d = texture(_LightsRenderTexture, in_f.texcoord.xy);
          
          u_xlat0_d = u_xlat0_d * float4(_Intensity);
          
          u_xlat0_d = min(u_xlat0_d, float4(float4(_MaximumBrightness, _MaximumBrightness, _MaximumBrightness, _MaximumBrightness)));
          
          u_xlat0_d = u_xlat0_d + float4(float4(_MinimumBrightness, _MinimumBrightness, _MinimumBrightness, _MinimumBrightness));
          
          u_xlat1 = texture(_MainTex, in_f.texcoord.xy);
          
          out_f.color = u_xlat0_d * u_xlat1;

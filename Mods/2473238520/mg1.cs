...
namespace Mod
{
    public class mg1 : CanShoot
    {
        ...
        public override void Shoot()
        {
            ...
            if (upgraded == false)
            {
                physicalBehaviour.rigidbody.AddForceAtPosition(dir * -5, pos, ForceMode2D.Impulse);
                var bolt = new GameObject("pellet", typeof(mg2));
        ...
        private IEnumerator Fire()
        {
            ...
            {
                var end = Physics2D.Raycast(BarrelPosition, BarrelDirection, 1000, bounds);
                var endPos = end.transform == null ? (Vector3)BarrelPosition + (Vector3)BarrelDirection * 1000 : (Vector3)end.point;
